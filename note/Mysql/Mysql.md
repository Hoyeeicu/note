...
方法一：运行mysql提供的客户端命令工具 mysql 8.0 command line client

输入密码

方法二：利用系统自带命令行工具，要提前添加环境变量

输入mysql -u root -p

输入密码

![image-20221025142049884](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/image-20221025142049884.png)
...
select * from employee where idcard like '__________________X';
```
...
例：
`SELECT count(id) from employee where workaddress = '北京';`
...
- 分页查询是数据库的方言，**不同数据库有不同实现**，ySQL是LIMIT
...
![image-20221026221832644](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/image-20221026221832644.png)
...
约束是作用于表中字段上的，可以再**==创建表/修改表==**的时候添加约束。
...
![image-20221028200937457](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/image-20221028200937457.png)
...
-- 查询员工姓名，及关联的部门的名称
-- 隐式
select e.name, e.name from employee as e, dept as d where e.dept = d.id;
-- 显式
...
```mysql
select salary from emp where name = '灭绝'; 1
```
...
![MySQL体系结构_20220315034329549927](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/MySQL%E4%BD%93%E7%B3%BB%E7%BB%93%E6%9E%84_20220315034329549927.png)
![MySQL体系结构层级含义_20220315034359342837](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/MySQL%E4%BD%93%E7%B3%BB%E7%BB%93%E6%9E%84%E5%B1%82%E7%BA%A7%E5%90%AB%E4%B9%89_20220315034359342837.png)

**存储引擎就是存储数据、建立索引、更新/查询数据等技术的实现方式。**存储引擎是基于表而不是基于库的，所以存储引擎也可以被称为表引擎。
![MySQL体系结构层级含义_20220315034359342837](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/MySQL%E4%BD%93%E7%B3%BB%E7%BB%93%E6%9E%84%E5%B1%82%E7%BA%A7%E5%90%AB%E4%B9%89_20220315034359342837-1667046475613-5.png)
...
- xxx.ibd: xxx代表表名，InnoDB 引擎的每张表都会对应这样一个表空间文件，存储该表的**表结构**（frm、sdi）、**数据**和**索引**。
...
InnoDB 逻辑存储结构：
![InnoDB逻辑存储结构](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/%E9%80%BB%E8%BE%91%E5%AD%98%E5%82%A8%E7%BB%93%E6%9E%84_20220316030616590001.png "InnoDB逻辑存储结构")
...
![二叉树](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/%E4%BA%8C%E5%8F%89%E6%A0%91_20220316153214227108.png "二叉树")

二叉树的缺点可以用红黑树来解决：
![红黑树](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/%E7%BA%A2%E9%BB%91%E6%A0%91_20220316163142686602.png "红黑树")
红黑树也存在大数据量情况下，层级较深，检索速度慢的问题。

为了解决上述问题，可以使用 B-Tree 结构。

B-Tree (多路平衡查找树) 以一棵**最大度数**（max-degree，指一个节点的子节点个数）为5（5阶）的 b-tree 为例（每个节点最多存储4个key，5个指针）

![B-Tree结构](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/B-Tree%E7%BB%93%E6%9E%84_20220316163813441163.png "B-Tree结构")
...
![B+Tree结构图](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/B%2BTree%E7%BB%93%E6%9E%84%E5%9B%BE_20220316170700591277.png "B+Tree结构图")
...
![MySQL B+Tree 结构图](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/%E7%BB%93%E6%9E%84%E5%9B%BE_20220316171730865611.png "MySQL B+Tree 结构图")
...
![Hash索引原理图](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/Hash%E7%B4%A2%E5%BC%95%E5%8E%9F%E7%90%86%E5%9B%BE_20220317143226150679.png "Hash索引原理图")
...
| 分类                      | 含义                                                         | 特点                 |
| ------------------------- | ------------------------------------------------------------ | -------------------- |
| 聚集索引(Clustered Index) | 将数据存储与索引放一块，索引结构的叶子节点保存了行数据(下面挂着这一行的数据) | 必须有，而且只有一个 |
| 二级索引(Secondary Index) | 将数据与索引分开存储，索引结构的叶子节点关联的是对应的主键(下面挂着对应id) | 可以存在多个         |

演示图：

![大致原理](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/%E5%8E%9F%E7%90%86%E5%9B%BE_20220318194454880073.png "大致原理")
![演示图](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/%E6%BC%94%E7%A4%BA%E5%9B%BE_20220319215403721066.png "演示图")
...
>
> 找。但是在二级索引中只能查找到 Arm 对应的主键值 10。②. 由于查询返回的数据是*，所以此时，还需要根据主键值10，到聚集索引中查找10对应的记录，最
>
> 终找到10对应的行row。
>
> ③. 最终拿到这一行的数据，直接返回即可。
...
如果不加 CREATE 后面不加索引类型参数，默认创建的是常规索引simple
...
2. 字符串类型字段使用时，不加单引号，索引将失效。

   如：`explain select * from tb_user where phone = 17799990015;`，此处phone的值没有加引号

3. 模糊查询中，如果仅仅是**尾部模糊匹配**，索引不会是失效；如果是**头部模糊匹配**，索引失效。

   如：`explain select * from tb_user where profession like '%工程';`索引失效，

   前后都有 % 也会失效。总结：**大数据尽量规避前模糊查询**
...
是优化数据库的一个重要手段，简单来说，就是在SQL语句中加入一些人为的提示来达到优化操作的目的。 就是告诉mysql用哪个索引
...
如果在聚集索引中直接能找到对应的行，则直接返回行数据，只需要一次查询，哪怕是select \*；如果在辅助索引(二级索引)中找聚集索引，

如`select id, name from xxx where name='xxx';`，也只需要通过辅助索引(name)查找到对应的id，返回name和name索引对应的id即可，只需要一次查询；

如果是通过辅助索引查找其他字段，则需要回表查询，如`select id, name, gender from xxx where name='xxx';`

所以尽量不要用`select *`，容易出现回表查询，降低效率，除非有联合索引包含了所有字段

![image-20221103162750060](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/image-20221103162750060.png)
...
应用环境：当字段类型为字符串（varchar, text等）时，有时候需要索引很长的字符串，这会让索引变得很大，查询时，浪费大量的磁盘IO，影响查询效率，此时可以只降字符串的一部分前缀，建立索引，这样可以大大节约索引空间，从而提高索引效率。
...
![image-20221103163648515](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/image-20221103163648515.png)
...
![image-20221103200333811](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/image-20221103200333811.png)
...
人员的很多工作，减少数据在数据库和应用服务器之间的传输，对于提高数据处理的效率是有好处的。存储过程思想上很简单，就是数据库 SQL 语言层面的代码封装与重用。

![image-20221103200959953](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/image-20221103200959953.png)
...
![image-20221103203557905](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/image-20221103203557905.png)

#### 常用工具（指令）

**mysql**

![image-20221103204110679](https://raw.githubusercontent.com/Hoyeeicu/note-blog/main/img/image-20221103204110679.png)
...
[93. 进阶-MySQL管理-常用工具2_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1Kr4y1i7ru?p=150&vd_source=a8443830a3622a0160f0871b629137bf)
...
课程视频:[01.MySQL课程介绍_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1Kr4y1i7ru?p=1&vd_source=a8443830a3622a0160f0871b629137bf)

相关资料文档:

linux基本操作指令:[(17条消息) 10分钟让你掌握Linux常用命令(+1.4万+++收藏)_ha_lydms的博客-CSDN博客_linux常用命令](https://blog.csdn.net/lydms/article/details/101368670?spm=1001.2014.3001.5506)

本机datagrip连接Linux服务器数据库:[08. 进阶-MySQL安装(linux版本)_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1Kr4y1i7ru?p=65&vd_source=a8443830a3622a0160f0871b629137bf)

云服务器:[云服务器 | 菜鸟教程 (runoob.com)](https://www.runoob.com/linux/linux-cloud-server.html)
...
2022/10/25——2022/11/3

实践周结束了，mysql学习也告一段落了，两周的时间还是学到了挺多东西的
